...
1. 安装 OpenCode 与 [`uv`](https://github.com/astral-sh/uv)，`UV_BIN` 可选。
...
4. 在 OpenCode 中照常使用该 skill。import 失败时再跑 `doctor`。
...
默认：`VENV_ROOT=~/.config/opencode/.venvs`，`SKILL_ROOT=~/.opencode/skills`。见 [`docs/python-isolation.zh-CN.md`](docs/python-isolation.zh-CN.md)。
